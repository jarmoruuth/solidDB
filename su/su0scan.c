 ...
 * Globals used : 
 */
bool su_sscandouble(src, p_d, p_mismatch)
	char* src;
	double* p_d;
        char** p_mismatch;
{
        ...
        if (valid) {
#ifdef SS_SOLARIS /* atof() in solaris can cause FPE's */
            ...
            SS_TRAP_HANDLERSECTION
                ...
                case SS_TRAP_FPE_EXPLICITGEN:
                    ...
                    valid = FALSE;
                    ss_dprintf_1(("su_scandouble FP trap %d\n", trapcode));
                    break;
                default:
                    SS_TRAP_RERAISE();
                    ss_error;
 ...
 * Globals used : 
 */
bool su_sscanlong(char* s, long* p_l, char** p_mismatch)
{
        bool valid;
        bool negative;
        ulong v;
        ...
        for (valid = FALSE; ; v = v * (ulong)10 + t, s++, valid = TRUE) {
            switch (*s) {
                case '9':
                    t = 9;
                    continue;
 ...
 * Globals used : 
 */
bool su_sscanint(src, p_i, p_mismatch)
	char* src;
	int* p_i;
        char** p_mismatch;
{
        ...
        if (su_sscanlong(src, &l, p_mismatch) != 0) {
            *p_i = (int)l;
            return(TRUE);
        } else {
            return(FALSE);
